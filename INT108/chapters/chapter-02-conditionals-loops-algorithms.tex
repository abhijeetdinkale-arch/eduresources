\chapter{Conditional Statements, Iterative Statements \& Algorithms}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item \textbf{State Update}: Modifying the control variable toward termination (e.g., \texttt{count += 1}). Missing this causes an \textbf{Infinite Loop}.
\end{enumerate}
\end{conceptbox}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item \textbf{The Loop \texttt{else} Clause}: An optional block attached to a \texttt{for} or \texttt{while} loop that executes \textbf{only if the loop completes normally without encountering a \texttt{break}}.
\end{enumerate}
...
When $i=1$, $i\%2 == 0$ is False. The \texttt{i += 1} inside the \texttt{if} block is skipped, so $i$ remains $1$ forever, resulting in an \textbf{infinite loop}. The increment \texttt{i += 1} must occur outside the \texttt{if} block:
\begin{lstlisting}[language=Python3]
total = 0
i = 1
while i <= 10:
...
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Level 1 (Foundation)}: Write a program that checks whether a given year is a \textbf{Leap Year} (divisible by 4, but not 100 unless also divisible by 400).
    ...
    \item \textbf{Level 2 (Standard)}: Write a program to check if an input integer is an \textbf{Armstrong Number} (e.g., $153 = 1^3 + 5^3 + 3^3$) using digit extraction.
...
\begin{tabular}{|l|l|l|}
\hline Input & Expected Output & Logic \\
\hline 3 & Rows: 1, 1 1, 1 2 1 & Binomial coefficients $N=3$ \\
\hline 1 & Row: 1 & Boundary $N=1$ \\
\hline
\end{tabular}
