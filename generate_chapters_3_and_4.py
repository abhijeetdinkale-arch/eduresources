...
# ==============================================================================
# CHAPTER 3: INTRODUCTION TO NUMBER SYSTEMS AND LOGIC GATES
# ==============================================================================
...
A Karnaugh Map is a 2D graphical representation of a truth table arranged in **Gray code order** ($00, 01, 11, 10$) so that adjacent cells differ by exactly one binary variable.
...
\begin{enumerate}
    ...
    \item **Prime Implicant (PI):** A rectangular group of $2^k$ 1-cells that cannot be merged into a larger valid group.
    \item **Essential Prime Implicant (EPI):** A Prime Implicant that contains at least one 1-cell not covered by any other prime implicant. All EPIs must appear in the minimal SOP expression.
    \item **Don't Care Conditions ($d$):** Input combinations that cannot occur or whose outputs are irrelevant may be treated as 1s (to enlarge groups) or 0s (to avoid creating extra groups).
\end{enumerate}
\end{conceptbox}
...
\end{itemize}
\textbf{Elimination Method:} On the K-map, static hazards occur between adjacent 1-cells covered by different prime implicant groups. Adding the **consensus prime implicant** $BC$ covers the transition:
\begin{equation}
...
print("Chapter 3 successfully written.")
...
A 4-input Priority Encoder accepts four inputs ($D_0, D_1, D_2, D_3$), where $D_3$ has the highest priority and $D_0$ has the lowest priority. It generates a 2-bit binary output ($Y_1, Y_0$) and a **Valid Bit** ($V$) indicating if any input is active.

\begin{table}[htbp]
\centering
\small
\caption{4-Input Priority Encoder Truth Table}
\label{tab:priority_encoder}
\begin{tabularx}{0.8\textwidth}{cccc|ccc}
\toprule
...
print("Chapter 4 successfully written.")
